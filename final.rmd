...
# for PDF no code
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, dev = "png", out.width = "100%", fig.align = "center", dpi = 300)
# for html with svg
...
```{r}
q <- query(
  "SELECT count(*) as count, race40, ager52, educ2003, sex
  FROM mortality 
  GROUP BY race40, ager52, educ2003, sex 
...
```{r plot_killer, fig.height = 5, fig.width = 8}
source("plots/killer.r", local = TRUE, print.eval = TRUE)
draw_killer(person_colors, 1) # this variable is defined globally
```

Middle aged people have on average the most records.
```{r table_age}
draw_table(age)
```
Not many large differences
```{r table_marstat}
draw_table(marital_status)
```
Accidents have by far the most records and pending investigation by far the least
```{r table_mandeath}
draw_table(manner)
```
```
...
```{r plot_weekday_deaths, fig.height = 5, fig.width = 8}
...
```{r plot_month_deaths, fig.height = 5, fig.width = 8}
source("plots/month_count.r", local = TRUE, print.eval = TRUE)

month_count_plot(df_month_counts)
```
...
```{r plot_month_manner, fig.height = 5, fig.width = 8}
month_count_table()
month_count_plot(df_month_counts_filtered, "Deaths per Day in Month Excluding Natural Causes")
...
```{r plot_age_deaths, fig.height = 5, fig.width = 8}
...
```{r plot_age_manner, fig.height = 5, fig.width = 8}
...
```{r plot_age_gender, fig.height = 5, fig.width = 8}
...
```{r plot_education_cause, fig.height = 5, fig.width = 8}
...
```{r plot_conditions_text, fig.height = 5, fig.width = 8}
